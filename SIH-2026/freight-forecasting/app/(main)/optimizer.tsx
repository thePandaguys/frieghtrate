  ...
  const run = async () => {
    ...
    try {
      const response = await predictVesselIdle({
        origin_port: 'Rotterdam', destination_port: 'Singapore', vessel_type: 'VLCC', cargo_quantity_mt: 85000,
        vessel_draft: 15.2, port_max_draft: 18, berth_count: 8, handling_rate_mt_hour: 2500,
        vessels_waiting: 4, port_congestion_index: 0.45, weather_index: 0.2, draft_clearance: 2.8,
        estimated_handling_hours: 34, queue_pressure: 0.5,
      });
  ...
  return (
    <ScreenShell title="Vessel Optimizer" subtitle="AI-powered vessel selection for every voyage">
      ...
      <Card style={styles.paramCard}>
        {[
          { label: 'ORIGIN', value: 'Rotterdam' },
          { label: 'DESTINATION', value: 'Singapore' },
          { label: 'CARGO', value: 'Crude Oil' },
          { label: 'QUANTITY', value: '85,000 MT' },
          { label: 'VESSEL CLASS', value: 'VLCC' },
          { label: 'CHARTER TYPE', value: 'Spot Charter' },
        ].map((p, i, arr) => (
          <View key={p.label} style={[styles.param, i < arr.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.divider }]}>
