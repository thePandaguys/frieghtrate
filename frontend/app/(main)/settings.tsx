...
const themeOptionsMeta: { mode: ThemeMode; label: string; icon: 'monitor' | 'sun' | 'moon' }[] = [
...
export default function Settings() {
  ...
  return (
    <ScreenShell title="Settings" subtitle="Configure intelligence, alerts and system preferences" badge="ACTIVE" badgeColor={colors.success}>
      ...
      <Card>
        <Text style={[styles.themeDesc, { color: colors.textSecondary }]}>Choose operational appearance across the application console.</Text>
        <View style={styles.themeOptions}>
          {themeOptionsMeta.map(opt => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  styles.themeOption,
                  {
                    backgroundColor: isSelected ? colors.cardHighlight : colors.cardAlt,
              ...
              >
                ...
                />
                <Text style={[styles.themeOptionText, { color: isSelected ? colors.text : colors.textSecondary }]}>
                  {opt.label}
                </Text>
                {isSelected && (
      ...
      <SettingCard icon="activity" title="Animations" description="Enable smooth transitions and intelligence motion" value={animations} onChange={setAnimations} />
      <Card style={styles.accentCard}>
        <View>
          <Text style={[styles.accentTitle, { color: colors.text }]}>Accent Color</Text>
          <Text style={[styles.accentDetail, { color: colors.textSecondary }]}>FREYNA Maritime Orange</Text>
        </View>
      ...
      <Card style={{ padding: 0 }}>
        {[
          ...
          { label: 'About App', value: 'FREYNA v1.0.0 (Command Console)' },
        ].map(r => <InfoRow key={r.label} label={r.label} value={r.value} />)}
      </Card>

      {/* System */}
      <SectionHeader eyebrow="System" title="System Preferences" />
      <SettingCard icon="refresh-cw" title="Automatic Data Refresh" description="Keep market and vessel intelligence synchronized" value={autoRefresh} onChange={setAutoRefresh} />
      <SettingCard icon="bell" title="Notifications" description="Receive important system and intelligence updates" value={notifications} onChange={setNotifications} />

      {/* Alerts */}
      <SectionHeader eyebrow="Intelligence" title="Alert Preferences" />
      <SettingCard icon="trending-up" title="Market Alerts" description="Notify when significant freight-rate movements occur" value={marketAlerts} onChange={setMarketAlerts} />
      <SettingCard icon="shield" title="Risk Alerts" description="Notify when operational or route risks are detected" value={riskAlerts} onChange={setRiskAlerts} />
      ...
      <Card style={[styles.modelCard, { borderColor: colors.primary }]}>
        <View style={styles.modelHeader}>
          <View>
            <Text style={[styles.modelEyebrow, { color: colors.primary }]}>ACTIVE MODEL</Text>
        ...
        {[
          { label: 'MODEL VERSION', value: 'OFX-4.2 (Production)' },
...
function SettingCard({ icon, title, description, value, onChange }: { icon: any; title: string; description: string; value: boolean; onChange: (v: boolean) => void }) {
  const { colors } = useTheme();
  return (
    <Card style={styles.settingCard}>
      <View style={[styles.settingIcon, { backgroundColor: colors.cardAlt, borderColor: colors.border }]}>
